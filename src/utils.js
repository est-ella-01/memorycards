...
export const categories=()=>{
  ...
  /*
1.  emojis.map((obj)=>obj.category) - csinál egy tömböt csak a kategóriákkal (az összessel)
2. new Set (emojis.map((obj)=>obj.category)) - csinál egy setet, amiben minden kategória csak 1× szerepel
3. Array.from(new Set (emojis.map((obj)=>obj.category))) - tömbbé alakítja a setet
4. Array.from(new Set (emojis.map((obj)=>obj.category))).sort() - abc sorrnedbe rendezi a tömb elemeit
   */
}
...
export const selectedEmojis=(ctg,nr)=>{
  let arr=emojis.filter((obj)=>obj.category==ctg) // új tömb, amiben csak a megadott kategóriába (ctg) tartozó objektumok vannak

  arr=arr.map((obj)=>{
    const {emoji,category}=obj
    return {emoji,category, disabled:false}
  }) // felülírjuk a tömböt, hogy az objektumok csak 2 attribútumok tartalmazzanak (destrukturáló szintaxissal)

  arr.sort(()=>Math.random()-0.5) // az elemek véletlen sorba rendezése

  arr=arr.slice(0,nr*nr/2) // az előső elemenk kiválasztása a megadott szám (nr) szerint

  arr=[...arr,...arr] // az elmek megkettőzése

  arr=arr.map((obj)=>({...obj,id:v4()}))

  return arr.sort(()=>Math.random()-0.5) // az elemek véletlen sorba rendezése 
